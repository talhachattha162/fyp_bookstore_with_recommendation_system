...
class _EditBookScreenState extends State<EditBookScreen> {
  ...
  //   final themeNotifier = Provider.of<ThemeNotifier>(context);
    ...
    builder: (context, selectedCategoryProvider, _) {
    ...
    return Consumer<ThemeNotifier>(
        builder: (context, themeNotifier, _) {
          return
                          ...
                          Consumer<FileNamesProvider>(builder: (context, fileNamesProvider, child) {
                            return Text(fileNamesProvider.filename1);
                          }),
                          const SizedBox(
                            height: 20,
                          ),
    ...
    ..showSnackBar(snackBar);
                                    ...
                                    fileNamesProvider.filename3 = "<2 mb image allowed";
                                     Provider.of<FreeRentPaidProvider>(context).updateFreeRentPaid('free', false);
                                    if (mounted) {
                                      setState(() {
                                        isLoading = false;
                                      });
